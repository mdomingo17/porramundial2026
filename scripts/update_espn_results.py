...
def tokens_from_espn_competitor(comp) -> set[str]:
    ...
    vals = [team.get("displayName"), team.get("shortDisplayName"), team.get("name"), team.get("location"), team.get("abbreviation")]
    out = {norm(v) for v in vals if v}
    for v in list(out):
        out |= {norm(a) for a in ALIASES.get(v, set())}
    return {v for v in out if v}
...
def event_finished(event: dict) -> bool:
    ...
    return bool(t.get("completed")) or norm(t.get("state")) == "post" or "full time" in norm(t.get("description"))
...
def main():
    ...
    old = json.loads(RESULTS_PATH.read_text(encoding="utf-8")) if RESULTS_PATH.exists() else {"matches": []}
    old_by_id = {m.get("match_id"): m for m in old.get("matches", [])}

    today = datetime.now(timezone.utc).date()
    start = max(START, today - timedelta(days=5))
    ...
    for match in predictions.get("schedule", []):
        ...
        if event and event_finished(event):
            ...
            if score:
                h, a, _, _ = score
                result.update({"home_goals": h, "away_goals": a, "status": "finished", "source": "ESPN public scoreboard", "espn_event_id": event.get("id")})
                finished_count += 1
                if not previous or previous.get("home_goals") != h or previous.get("away_goals") != a or previous.get("status") != "finished":
                    changed += 1
            else:
            ...
            result.update({"home_goals": None, "away_goals": None, "status": "scheduled", "source": "ESPN public scoreboard"})
